...
pub struct RelaySession {
    ...
    /// Virtual UDP socket backed by the relay bind stream.
    pub relay_socket: Option<Arc<crate::masque::MasqueRelaySocket>>,
...
pub struct NatTraversalEndpoint {
    ...
    /// Shared QUIC endpoint bound to the relay socket.
    ///
    /// The relay server allows one CONNECT-UDP bind session per client address,
    /// so callers must reuse the same abstract socket/endpoint instead of
    /// trying to create a second independent bind session on the same local
    /// QUIC endpoint.
...
struct NatTraversalSession {
    ...
    /// Last traversal progress timestamp
    last_progress_at: std::time::Instant,
    /// Authoritative next protocol deadline for this session
    next_deadline: Option<SessionDeadline>,
    /// Stored retry schedule, if any
    retry_at: Option<std::time::Instant>,
    /// Last classified failure for this session
...
impl NatTraversalEndpoint {
    ...
    pub(crate) async fn setup_proactive_relay(
        ...
        let (public_addr, relay_endpoint) =
            self.ensure_shared_relay_endpoint(bootstrap_addr).await?;
        ...
        // Step 2: Spawn the accept loop exactly once for the shared relay endpoint.
        if !self
            .relay_accept_loop_started
            .swap(true, std::sync::atomic::Ordering::Relaxed)
        {
            info!(
                "Shared relay endpoint ready for accepting connections at {}",
                relay_public_addr
            );
    ...
    fn select_best_relay_public_address_for_family<I>(
        candidates: I,
        want_ipv4: bool,
    ) -> Option<SocketAddr>
    where
        I: IntoIterator<Item = SocketAddr>,
    {
        let mut addresses: Vec<_> = candidates
            ...
            .filter(|addr| !addr.ip().is_unspecified() && addr.is_ipv4() == want_ipv4)
            .filter(|addr| {
                matches!(
                    crate::reachability::socket_addr_scope(*addr),
                    Some(crate::reachability::ReachabilityScope::Global)
    ...
    fn select_relay_server_public_addresses(
        candidates: &[SocketAddr],
    ) -> (Option<SocketAddr>, Option<SocketAddr>) {
        (
            Self::select_best_relay_public_address_for_family(candidates.iter().copied(), true),
            Self::select_best_relay_public_address_for_family(candidates.iter().copied(), false),
        )
    }
    ...
    fn best_observed_external_address_for_family(&self, want_ipv4: bool) -> Option<SocketAddr> {
        ...
        if let Ok(addrs) = self.test_observed_external_addrs.lock()
            && let Some(addr) =
                Self::select_best_relay_public_address_for_family(addrs.iter().copied(), want_ipv4)
        {
            return Some(addr);
        }
        ...
        let known_peer_candidates: Vec<_> = self
            .connections
            .iter()
        ...
        match &envelope.message {
                ...
                if !normalized_initiator_addrs.is_empty() && !normalized_target_addrs.is_empty() {
                    ...
                    let accepted = CoordinatorControlEnvelope {
                        ...
                        message: CoordinatorControlMessage::CoordinationAccepted {
                            ...
                            target: *target,
                            round: *round,
                            initiator_addrs: normalized_initiator_addrs.clone(),
            ...
            CoordinatorControlMessage::CoordinationAccepted {
                ...
                if *initiator == self.local_peer_id() {
                    ...
                    let endpoint = self.inner_endpoint.clone().ok_or_else(|| {
                    ...
                    let connection_timeout = self
                        ...
                        .nat_traversal
                        .connection_establishment_timeout;
                    let candidate_addrs = normalized_target_addrs.clone();
                    ...
                    tokio::spawn(async move {
                        ...
                        for addr in candidate_addrs {
                            ...
                            .await
                            {
                                ...
                                Err(error) => {
                                    debug!(
                                        "Background validated connection attempt to {} for peer {:?} failed: {}",
                                        addr, target_peer, error
                                    );
                        ...
                        if let Some(error) = last_error {
                            warn!(
                                "Failed to establish any accepted direct path for peer {:?}: {}",
                                target_peer, error
                            );
                        }
                    });
                    ...
                    if let Some(offer) = inbound_offer(self.local_peer_id(), envelope.request_id) {
                        ...
                        let mut offered_initiator_addrs = offer
                            .initiator_addrs
                            .iter()
                            .copied()
                            .map(normalize_socket_addr)
                            .collect::<Vec<_>>();
                        offered_initiator_addrs.sort_unstable();
                        offered_initiator_addrs.dedup();
                        if offered_initiator_addrs != normalized_initiator_addrs {
                            return Ok(true);
                        }
                        ...
                        tokio::spawn(async move {
                            ...
                            for addr in normalized_initiator_addrs {
                                ...
                                .await
                                {
                                    ...
                                    Err(error) => {
                                        debug!(
                                            "Background inbound accepted connection attempt to {} for peer {:?} failed: {}",
                                            addr, initiator_peer, error
                                        );
                                        last_error = Some(error);
                                    }
                                }
                            ...
                            if let Some(error) = last_error {
                                warn!(
                                    "Failed to establish any inbound accepted direct path for peer {:?}: {}",
                                    initiator_peer, error
                                );
                            }
                        });
                    }
    ...
    pub fn reset_relay_state(&self) {
        ...
        self.relay_accept_loop_started
            .store(false, std::sync::atomic::Ordering::Relaxed);
        if let Ok(mut addr) = self.relay_public_addr.lock() {
            *addr = None;
        }
        if let Ok(mut endpoint) = self.shared_relay_endpoint.lock() {
            *endpoint = None;
        }
        if let Ok(mut peers) = self.relay_advertised_peers.lock() {
            peers.clear();
    ...
    /// Receive a peer address update originating from an ADD_ADDRESS frame.
    pub async fn recv_peer_address_update(&self) -> Option<(SocketAddr, SocketAddr)> {
        self.peer_address_update_rx.lock().await.recv().await
    }

    /// Get or create the shared QUIC endpoint that runs over the relay socket.
    pub(crate) async fn ensure_shared_relay_endpoint(
        &self,
        relay_addr: SocketAddr,
    ) -> Result<(Option<SocketAddr>, InnerEndpoint), NatTraversalError> {
        let relay_addr = normalize_socket_addr(relay_addr);
            ...
            .and_then(|guard| guard.clone())
        {
            let public_addr = self
                .relay_sessions
                .get(&relay_addr)
                .and_then(|session| session.public_address)
                .or_else(|| self.relay_public_addr());
            return Ok((public_addr, endpoint));
        }

        let (public_addr, relay_socket) = self.establish_relay_session(relay_addr).await?;
        ...
        let mut relay_endpoint = InnerEndpoint::new_with_abstract_socket(
        ...
        if let Some(client_config) = self
            .inner_endpoint
            .as_ref()
            .and_then(|endpoint| endpoint.default_client_config.clone())
        {
    ...
    > {
        ...
        if let Some(session) = self.relay_sessions.get(&relay_addr) {
            if session.is_active() {
                ...
                return Ok((session.public_address, session.relay_socket.clone()));
            }
        }
    ...
    fn store_successful_candidate_pair(
        ...
        if let Some(mut session) = self.active_sessions.get_mut(&peer_id) {
            for candidate in &mut session.candidates {
                if normalize_socket_addr(candidate.address)
                    == normalize_socket_addr(pair.remote_candidate.address)
                {
                    candidate.state = CandidateState::Validating;
                    break;
                }
        ...
        if let Some(ref callback) = self.event_callback {
            callback(NatTraversalEvent::CandidateValidated {
                peer_id,
                candidate_address: pair.remote_candidate.address,
            });
        }
    ...
    fn poll_discovery_manager(&self, now: std::time::Instant, events: &mut Vec<NatTraversalEvent>) {
        ...
        for discovery_event in discovery_events {
            ...
            if self.discovery_retry_backoff_active(current_peer_id, now) {
                continue;
            }
        ...
        for peer_id in early_discovery_peers {
            ...
            if !discovered_candidates.is_empty() {
                if let Some(mut entry) = self.active_sessions.get_mut(&peer_id) {
                    ...
                    Self::set_session_phase(session, now, TraversalPhase::Coordination);
                }
                self.emit_event(
                    &mut events,
    ...
    fn recompute_session_deadline(
        ...
        match session.phase {
            TraversalPhase::Discovery => self
                .discovery_manager
                .lock()
                .phase_timeout_deadline_for_peer(session.peer_id)
                .map(|at| SessionDeadline {
                    kind: TraversalDeadlineKind::DiscoveryProgress,
                    at,
                }),
            TraversalPhase::Coordination => Some(SessionDeadline {
                kind: TraversalDeadlineKind::CoordinationResponse,
                at: session.last_progress_at + self.config.coordination_timeout,
            }),
            TraversalPhase::Synchronization => Some(SessionDeadline {
                kind: TraversalDeadlineKind::SynchronizationProgress,
                at: session.last_progress_at + self.config.coordination_timeout,
            }),
            TraversalPhase::Punching => Some(SessionDeadline {
                kind: TraversalDeadlineKind::PunchProgress,
                at: session.last_progress_at + self.connectivity_phase_budget(session),
            }),
            TraversalPhase::Validation => Some(SessionDeadline {
                kind: TraversalDeadlineKind::ValidationProgress,
                at: session.last_progress_at + self.connectivity_phase_budget(session),
            }),
    ...
    fn retry_disposition(
        ...
        if attempt >= self.config.max_concurrent_attempts as u32 {
        ...
        match reason {
            ...
            | TraversalFailureReason::CoordinationRejected { .. } => RetryDisposition::Never,
            ...
            | TraversalFailureReason::NetworkError(_) => {
                RetryDisposition::After(now + self.calculate_backoff(attempt + 1))
            }
        }
    }
    ...
    fn check_punch_results_for_session(&self, session: &NatTraversalSession) -> Option<SocketAddr> {
        ...
        if let Some(addr) = self
            ...
            .filter(|addr| {
                session.candidates.iter().any(|candidate| {
                    normalize_socket_addr(candidate.address) == normalize_socket_addr(*addr)
                })
            })
        {
            info!(
                "Found successful hole punch candidate for peer {:?} at {}",
                peer_id, addr
            );
            return Some(addr);
        }
        ...
        debug!("No successful punch results for peer {:?}", peer_id);
        None
    }
    ...
    fn discovery_retry_backoff_active(&self, peer_id: PeerId, _now: std::time::Instant) -> bool {
        self.active_sessions.get(&peer_id).is_some_and(|entry| {
            let session = entry.value();
            matches!(session.phase, TraversalPhase::Discovery)
                && matches!(
                    session.next_deadline.as_ref(),
                    Some(SessionDeadline {
                        kind: TraversalDeadlineKind::RetryBackoff,
                        ..
                    })
                )
    ...
    fn get_current_discovery_peer_id(&self) -> PeerId {
        ...
        // If no discovery phase session is active, discovery events describe our
        // local address set rather than a remote peer candidate set.
        self.local_peer_id
    }
    ...
    async fn establish_connection_to_validated_candidate(
        ...
        let endpoint = self.inner_endpoint.as_ref().cloned().ok_or_else(|| {
            NatTraversalError::ConfigError("QUIC endpoint not initialized".to_string())
        })?;

        Self::establish_connection_to_validated_candidate_inner(
            endpoint,
    ...
    async fn establish_connection_to_validated_candidate_inner(
        endpoint: InnerEndpoint,
        active_sessions: Arc<dashmap::DashMap<PeerId, NatTraversalSession>>,
        connections: Arc<dashmap::DashMap<PeerId, InnerConnection>>,
        emitted_established_events: Arc<dashmap::DashSet<PeerId>>,
        ...
        let connection = timeout(connection_timeout, connecting)
            .await
            .map_err(|_| NatTraversalError::Timeout)?
            .map_err(|e| NatTraversalError::ConnectionFailed(format!("Connection failed: {e}")))?;
        ...
        if let Some(mut session) = active_sessions.get_mut(&peer_id) {
            Self::set_session_phase(
                session.value_mut(),
                std::time::Instant::now(),
                TraversalPhase::Connected,
...
mod tests {
    ...
    #[test]
    fn test_best_relay_public_address_ignores_private_candidates() {
        let private_v4: SocketAddr = "10.0.0.1:5483".parse().expect("valid addr");
        let global_v4: SocketAddr = "198.51.100.44:5483".parse().expect("valid addr");
        let global_v6: SocketAddr = "[2001:db8::44]:5483".parse().expect("valid addr");

        assert_eq!(
            NatTraversalEndpoint::select_best_relay_public_address_for_family(
                [private_v4, global_v4],
                true,
            ),
            Some(global_v4)
        );
    ...
    #[test]
    fn test_session_is_synchronized_requires_candidates() {
        ...
        assert!(
            !NatTraversalEndpoint::session_is_synchronized(&session),
            "Synchronization requires at least one candidate"
        );
    }

    #[tokio::test]
    async fn test_check_punch_results_requires_real_success() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        assert_eq!(endpoint.check_punch_results_for_session(&session), None);

        endpoint.shutdown().await.expect("Shutdown should succeed");
    }

    #[tokio::test]
    async fn test_check_punch_results_uses_recorded_successful_candidate() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        let candidate_addr: SocketAddr = "127.0.0.1:45555".parse().unwrap();
        endpoint
    ...
    async fn test_connected_phase_has_no_contract_timeout_deadline() {
        ...
        assert!(
            endpoint
                .recompute_session_deadline(&session, started_at)
                .is_none()
        );
        endpoint.shutdown().await.expect("Shutdown should succeed");
    }
    ...
    #[tokio::test]
    async fn test_recompute_session_deadline_prefers_retry_backoff() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
        ...
        let session = NatTraversalSession {
            ...
            last_failure: Some(TraversalFailureReason::NetworkError(
                "temporary".to_string(),
            )),
            session_state: SessionState {
                state: ConnectionState::Connecting,
        ...
        assert_eq!(deadline.kind, TraversalDeadlineKind::RetryBackoff);
        assert_eq!(deadline.at, retry_at);

        endpoint.shutdown().await.expect("Shutdown should succeed");
    }

    #[tokio::test]
    async fn test_recompute_session_deadline_uses_session_owned_non_discovery_budget() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
        ...
        let now = std::time::Instant::now();
        let last_progress_at = now - Duration::from_millis(400);
        let session = NatTraversalSession {
            peer_id: PeerId([0x72; 32]),
            coordinator: "127.0.0.1:9000".parse().unwrap(),
            attempt: 1,
        ...
        assert_eq!(
            deadline.kind,
            TraversalDeadlineKind::SynchronizationProgress
        );
        assert_eq!(deadline.at, last_progress_at + coordination_timeout);

        endpoint.shutdown().await.expect("Shutdown should succeed");
    }

    #[test]
    fn test_set_session_phase_clears_stale_retry_state_and_resets_progress_timestamp() {
        let original_now = std::time::Instant::now();
        let transition_now = original_now + Duration::from_secs(2);
        let mut session = NatTraversalSession {
            peer_id: PeerId([0x76; 32]),
            coordinator: "127.0.0.1:9000".parse().unwrap(),
            attempt: 2,
            ...
            next_deadline: Some(SessionDeadline {
                kind: TraversalDeadlineKind::RetryBackoff,
                at: original_now + Duration::from_secs(1),
            }),
            retry_at: Some(original_now + Duration::from_secs(1)),
            last_failure: Some(TraversalFailureReason::CoordinatorUnavailable),
            session_state: SessionState {
                state: ConnectionState::Connecting,
                last_transition: original_now,
                connection: None,
                active_attempts: Vec::new(),
                metrics: ConnectionMetrics::default(),
            },
        ...
        NatTraversalEndpoint::set_session_phase(
            &mut session,
            transition_now,
            TraversalPhase::Synchronization,
        );
        ...
        assert!(
            ...
            "phase progress should clear stale retry scheduling"
        );
        assert!(
            session.next_deadline.is_none(),
            "phase progress should clear stale retry deadline ownership"
        );
        assert_eq!(session.session_state.last_transition, transition_now);
    }

    #[tokio::test]
    async fn test_retry_disposition_distinguishes_retryable_vs_terminal_reasons() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        let now = std::time::Instant::now();
        let retryable =
            endpoint.retry_disposition(&TraversalFailureReason::CoordinatorUnavailable, 1, now);
        let terminal = endpoint.retry_disposition(
            &TraversalFailureReason::ProtocolViolation("bad frame".to_string()),
            1,
            now,
        );

        assert!(matches!(retryable, RetryDisposition::After(at) if at > now));
        assert!(matches!(terminal, RetryDisposition::Never));

        endpoint.shutdown().await.expect("Shutdown should succeed");
    }

    #[tokio::test]
    async fn test_handle_phase_failure_stores_retry_state_for_retryable_failures() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        let retry_at = session.retry_at.expect("retry scheduled");
        assert!(retry_at > now);
        assert!(matches!(
    ...
    #[tokio::test]
    async fn test_handle_phase_failure_classifies_synchronization_timeout_as_retryable_expiration()
    {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        ...
        endpoint.handle_phase_failure(&mut session, now, &mut events, NatTraversalError::Timeout);
        ...
        assert!(
            session.retry_at.is_some(),
            "synchronization timeout should be retryable"
        );
        assert!(matches!(
            session.next_deadline,
            Some(SessionDeadline {
    ...
    #[tokio::test]
    async fn test_handle_phase_failure_stores_retry_schedule_for_discovery_exhaustion() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        assert!(
            session.retry_at.is_some(),
            "discovery exhaustion should persist retry scheduling"
        );
        assert!(matches!(
            session.next_deadline,
            Some(SessionDeadline {
    ...
    #[tokio::test]
    async fn test_handle_phase_failure_emits_terminal_event_and_clears_retry_state() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            ..Default::default()
        };
        ...
        endpoint.handle_phase_failure(
            ...
            now,
            &mut events,
            NatTraversalError::ProtocolError("malformed control message".to_string()),
        );

        assert!(matches!(
            session.last_failure,
            Some(TraversalFailureReason::ProtocolViolation(_))
        ));
        assert!(
            session.retry_at.is_none(),
            "terminal failures clear retry_at"
        );
        assert!(
            session.next_deadline.is_none(),
    ...
    #[tokio::test]
    async fn test_coordinator_request_expiry_helper_is_bounded_by_session_budget_and_timeout() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            coordination_timeout: Duration::from_secs(5),
            ..Default::default()
        ...
        assert!(
            expiry_duration <= Duration::from_secs(4),
            "expiry should clamp to remaining session-owned budget"
        );
    ...
    #[tokio::test]
    async fn test_coordinator_request_expiry_helper_applies_minimum_floor() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            coordination_timeout: Duration::from_secs(5),
            ..Default::default()
    ...
    #[tokio::test]
    async fn test_discovery_retry_backoff_is_preserved_until_due_and_then_consumed() {
        let config = NatTraversalConfig {
            bind_addr: Some("127.0.0.1:0".parse().unwrap()),
            coordination_timeout: Duration::from_secs(5),
            ..Default::default()
        ...
        let retry_at = base_now + Duration::from_secs(30);
        endpoint.active_sessions.insert(
            peer_id,
            NatTraversalSession {
                peer_id,
                ...
                retry_at: Some(retry_at),
                last_failure: Some(TraversalFailureReason::DiscoveryExhausted),
                session_state: SessionState {
                    state: ConnectionState::Connecting,
                    last_transition: base_now,
                    connection: None,
                    active_attempts: Vec::new(),
                    metrics: ConnectionMetrics::default(),
                },
        ...
        let events_before = endpoint
            .poll(base_now)
            .expect("poll before retry deadline should succeed");
        assert!(
            events_before.is_empty(),
            "inert retry wait should not emit events"
        );
        {
            let session = endpoint
                .active_sessions
                .get(&peer_id)
                .expect("session before due");
            assert_eq!(session.retry_at, Some(retry_at));
            assert!(matches!(
                session.next_deadline,
                Some(SessionDeadline {
                    kind: TraversalDeadlineKind::RetryBackoff,
        ...
        );
        {
            let session = endpoint
                .active_sessions
                .get(&peer_id)
                .expect("session after due");
            assert!(
                session.retry_at.is_none(),
                "retry state should be consumed once due"
            );
            assert!(
                !matches!(
                    session.next_deadline,
